...
import toggleScrollbar from '@/ds/utils/scrollbar'
import {
  autoUpdate,
  computePosition,
  flip,
...
export default class Positionable {
  ...
  private declare component: AlpineComponent
  ...
  watch(callback: (value: boolean) => void): this {
    queueMicrotask(() => {
      this.component.$watch('positionable.state', (value: boolean) =>
        callback(value),
      )
    })
